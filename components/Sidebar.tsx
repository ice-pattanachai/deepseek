  ...
  return (
    ...
    ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
        <div>
            <div className={`flex ${expand ? "flex-row gap-10 items-center" : "flex-col items-center gap-8"}`}>
                ...
                className={`
                    ...
                    <Image src={assets.menu_icon} alt='Menu Icon' className={`md:hidden w-5 h-5`} />
                    <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} alt={expand ? 'Close Sidebar' : 'Open Sidebar'}
                        className={`hidden md:block w-7`} /> 

                    {/* Tooltip */}
                    <div className={`absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12" : "-top-12 left-1/2 -translate-x-1/2"} {/* แก้ไขตำแหน่ง tooltip เมื่อ collapsed */}
                    opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black text-white text-sm px-3 py-2 rounded-lg
                    shadow-lg pointer-events-none`}>
                        {expand ? 'Close Sidebar' : 'Open Sidebar'}

                        <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "left-1/2 -translate-x-1/2 -top-1.5" : "left-1/2 -translate-x-1/2 -bottom-1.5"}`}> {/* แก้ไขตำแหน่ง arrow */}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div>
            ...
            <div className={`flex items-center ${expand ? 'hover:bg-white/10 rounded-lg' : 'justify-center w-full'} 
