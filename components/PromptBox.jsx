...
const PromptBox= ({ setIsLoading, isLoading }) => {
    ...
    const sendPrompt = async (e) => {
        ...
        try {
            ...
            // ตรวจสอบว่ามี chats และ selectedChat หรือไม่
            if (!selectedChat || !selectedChat._id) {
                return toast.error('No selected chat available.');
            }
    ...
    return (
        ...
        className={`w-full ${selectedChat?.messages?.length > 0 ? "max-w-3xl" : "max-w-2xl"} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
            ...
            className='outline-none w-full resize-none overflow-hidden break-words bg-transparent' rows={2}
